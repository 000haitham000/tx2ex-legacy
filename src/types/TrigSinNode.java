 ...
 * @author haitham
 */
public class TrigSinNode extends AbstractNode {

    public TrigSinNode(AbstractNode parent, List<Token> allTokens) {
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("sin")
                    ...
                    if (last == -1) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Un-balanced parentheses, Sine closing bracket ')' is missing.");
                    } else {
                        // List of lists of tokens (will only contain one
                        // list here as TrigSinNode has only one child)
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new TrigSinNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("sin(%s)", getChild(0).toString());
    }

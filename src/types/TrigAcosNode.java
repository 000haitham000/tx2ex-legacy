 ...
 * @author haitham
 */
public class TrigAcosNode extends AbstractNode {

    public TrigAcosNode(AbstractNode parent, List<Token> allTokens) {
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("acos")
                    ...
                    if (last == -1) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Un-balanced parentheses, ArcCosine closing bracket ')' is missing.");
                    } else {
                        // List of lists of tokens (will only contain one
                        // list here as TrigAcosNode has only one child)
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new TrigAcosNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("acos(%s)", getChild(0).toString());
    }

...
public class SqrtNode extends AbstractNode {
    ...
    public SqrtNode(AbstractNode parent, List<Token> allTokens) {
        super(parent, allTokens);
    }

    @Override
    public double evaluate() throws EvaluationException {
        return Math.sqrt(getChild(0).evaluate());
    }
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("sqrt")
                    ...
                    if (last == -1) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Un-balanced parentheses, Sqrt closing bracket ')' is missing.");
                    } else {
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new SqrtNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("sqrt(%s)", getChild(0).toString());
    }

 ...
 * @author haitham
 */
public class TrigAsinNode extends AbstractNode {

    public TrigAsinNode(AbstractNode parent, List<Token> allTokens) {
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("asin")
                    ...
                    if (last == -1) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Un-balanced parentheses, ArcSine closing bracket ')' is missing.");
                    } else {
                        // List of lists of tokens (will only contain one
                        // list here as TrigAsinNode has only one child)
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new TrigAsinNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("asin(%s)", getChild(0).toString());
    }

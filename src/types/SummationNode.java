 ...
 * @author haitham
 */
public class SummationNode extends AbstractMultiOpNode {

    private VariablesManager varManager;
    private String indexLabel;

    public SummationNode(AbstractNode parent, List<Token> allTokens, VariablesManager varManager) {
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("sum")) {
                    // the keyword "sum" must be followed by "{"
                    if (targetTokens.get(i + 1).getString().equals("{")) {
                        ...
                        if (last == targetTokens.size()) {
                            ...
                            throw new InvalidFormatException(
                                    targetTokens.get(i),
                                    allTtokens,
                                    "Summation operator closing bracket '}' is missing.");
                        } else {
                            ...
                            if (comma1Pos == -1 || comma2Pos == -1 || comma3Pos == -1) {
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "Summation operator needs four operands (index label, start value, end value and expression), separated by three commas.");
                            }
                            if (comma1Pos - (i + 1) < 2) {
                            ...
                            if (comma1Pos - (i + 1) > 2) {
                                ...
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "Summation operator first operand must be only one token, which is a valid identifier representing index label.");
                            }
                            if (comma2Pos - comma1Pos < 2) {
                                ...
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "The second operand of the summation operator (index start value) is missing.");
                            }
                            if (comma3Pos - comma2Pos < 2) {
                                ...
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "The third operand of the summation operator (index end value) is missing.");
                            }
                            // Check for fourth and fifth operators existence
                            // depends on whether the summation operator being
                            // parsed has four or five arguments.
                            if (comma4Pos == -1) {
                                // If the summation operator being parsed has
                                ...
                                if (last - comma3Pos < 2) {
                                    ...
                                    throw new InvalidFormatException(
                                            targetTokens.get(i),
                                            allTtokens,
                                            "The fourth operand of the summation operator (expression) is missing.");
                                }
                            } else {
                                // If the summation operator being parsed has
                                ...
                                if (comma4Pos - comma3Pos < 2) {
                                    ...
                                    throw new InvalidFormatException(
                                            targetTokens.get(i),
                                            allTtokens,
                                            "The fourth operand of the summation operator (step) is missing.");
                                }
                                if (last - comma4Pos < 2) {
                                    // Missing fourth operand (index end)
                                    throw new InvalidFormatException(
                                            targetTokens.get(i),
                                            allTtokens,
                                            "The fifth operand of the summation operator (expression) is missing.");
                                }
                            }
                            if (!targetTokens.get(i + 2).getString().matches("[a-zA-Z_$][a-zA-Z\\d_$]*")) {
                                ...
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "Summation operator index label must be a valid identifier.");
                            }
                            // Now let's do the actual parsing! (0_0)
                            // List of lists of tokens (will contain three
                            // lists here as summation has three children
                            // expressions i.e. indexStart, indexEnd and the
                            // expression of to be summed)
    ...
    public String toString() {
        ...
        sb.append(String.format("Sum{%s=[%s,%s],", indexLabel, getChild(0).toString(), getChild(1).toString()));
        if(getChildrenCount() == 3) {

 ...
 * @author haitham
 */
public class CeilNode extends AbstractNode {

    public CeilNode(AbstractNode parent, List<Token> allTokens) {
        super(parent, allTokens);
    }

    @Override
    public double evaluate() throws EvaluationException {
        return Math.ceil(getChild(0).evaluate());
    }
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("ceil")
...
//                    }
                    ...
                    if (last == targetTokens.size()) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Ceiling operator closing bracket ')' is missing.");
                    } else {
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new CeilNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("ceil(%s)", getChild(0).toString());
    }

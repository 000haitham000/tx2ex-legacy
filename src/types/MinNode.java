 ...
 * @author haitham
 */
public class MinNode extends AbstractMultiOpNode {
    ...
    public double evaluate() throws EvaluationException {
        double min = getChild(0).evaluate();
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("min")) {
                    // the keyword "min" must be followed by "("
...
//                      }
                        ...
                        if (last == targetTokens.size()) {
                            ...
                            throw new InvalidFormatException(
                                    targetTokens.get(i),
                                    allTtokens,
                                    "Min operator closing bracket ')' is missing.");
                        } else {
                            ...
                            // min operator (which although useless is permitted).
                            // First, collect commas indices
                            List<Integer> commaIndices = new ArrayList<>();
                            for (int k = i + 2; k < last; k++) {
                                if ( // The first condition must be there to support nesting other operators
                                        // involving commas (like summations, products, max, other min operators,
                                        // etc.) inside min operator.
                            ...
                            // child is missing e.g. min(,b) or min(a,) or
                            // min(a,,c).
                            if (commaIndices.get(0) == i + 2) {
                                // First child is missing, like in min(,b)
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "The first operand of the min operator is missing, min(?,...).");
                                ...
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "Some operand of the min operator is missing, min(...,?,...).");
                            } else if (commaIndices.get(commaIndices.size() - 1) == last - 1) {
                                // Last child is missing, like in min(a,)
                                throw new InvalidFormatException(
                                        targetTokens.get(i),
                                        allTtokens,
                                        "The last operand of the min operator is missing, min(...,?).");
                            } else {
                                // Reaching this point means that we have the
                                // correct format i.e. min(a,b,c,...)
                                // Now let's do the actual parsing! (0_0)
                                // Create a list of lists of tokens (will 
                                // contain an arbitrary number of lists as min

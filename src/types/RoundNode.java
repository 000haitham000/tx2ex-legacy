 ...
 * @author haitham
 */
public class RoundNode extends AbstractNode {

    public RoundNode(AbstractNode parent, List<Token> allTokens) {
        super(parent, allTokens);
    }

    @Override
    public double evaluate() throws EvaluationException {
        return Math.round(getChild(0).evaluate());
    }
    ...
    public static NodeRecognizedInfo findLastOccurence(
        ...
        for (int i = targetTokens.size() - 2; i >= 0; i--) {
            ...
            if (!Utils.surrounded(targetTokens.get(i), targetTokens)) {
                ...
                if (targetTokens.get(i).getString().equalsIgnoreCase("round")
...
//                    }
                    ...
                    if (last == targetTokens.size()) {
                        ...
                        throw new InvalidFormatException(
                                targetTokens.get(i),
                                allTtokens,
                                "Round operator closing bracket ')' is missing.");
                    } else {
                        // List of lists of tokens (will only contain one
                        // list here as TrigTanNode has only one child)
                        ...
                        return new NodeRecognizedInfo(
                                i,
                                last,
                                new RoundNode(parent, allTtokens),
                                childrenTokensLists);
                    }
                }
            }
    ...
    public String toString() {
        return String.format("round(%s)", getChild(0).toString());
    }
